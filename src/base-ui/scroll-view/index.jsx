...
const ScrollView = memo((props) => {
    ...
    const contentRef = useRef() // -- 获取 content 容器元素
    ...
    // -- 事件监听函数
    function controlClickHandle(isRight) { // -- 监听点击向左/向右点击事件
        const newIndex = isRight ? posIndex.current + 1 : posIndex.current - 1 // -- 获取将要移动的元素 index 索引（根据 type 判断是向左还是向右移动）

        const newEl = contentRef.current.children[newIndex] // -- 获取对应的元素
    ...
    return (
        <ViewWrapper>
            ...
            <div className="scroll">
                <div className="sroll-content" ref={contentRef}>
                    {props.children}
                </div>
            </div>
        </ViewWrapper>
